...
def testPost():
    ...
    client = MongoClient(uri,
                        tls=True,
                        tlsCertificateKeyFile='C:/Users/hongd/pj/flask_react/admin_user.pem',
...
def testPost2():
    ...
    # parse string into two parts (zip code,n/t)
    postalCode = str(zip2)
    list = postalCode.split(",")
    postalCode = list[0] 
    tool = list[1]
    ...
    req = requests.get("http://simbad.u-strasbg.fr/simbad/sim-sam?Criteria=ra+%3E+"+ramin+"+%26+ra+%3C+"+ramax+"%0D%0A%26+dec+%3E+"+decmin+"+%26+dec+%3C+"+decmax+"%0D%0A%26+Vmag+%3C+"+limitingmag+"&submit=submit+query&OutputMode=LIST&maxObject=100&output.format=ASCII")
